...
def create_dxf(
    ...
    Args:
        boundary_poly: outer panel boundary (single or list for multi-copy sheets)
        safe_poly:     inner safe-margin boundary (single or list)
    ...
    ref_poly = boundary_list[0]  # used for label height fallback
    ...
    for bp in boundary_list:
        # ── REFERENCE_BOUNDARY ──────────────────────────────────────────────
        _add_geometry(msp, bp, "REFERENCE_BOUNDARY")
        # ── CUT_OUTER ───────────────────────────────────────────────────────
        _add_geometry(msp, bp, "CUT_OUTER")

    for sp in safe_list:
        # ── SAFE_MARGIN_GUIDE ───────────────────────────────────────────────
        _add_geometry(msp, sp, "SAFE_MARGIN_GUIDE")
    ...
    # ── ENGRAVE_LABEL ───────────────────────────────────────────────────────
    for lbl in labels:
        height = lbl.get("height", _default_label_height(ref_poly))
        text = msp.add_text(
            lbl["text"],
            dxfattribs={
                "layer": "ENGRAVE_LABEL",
