...
/** POST /api/preview — lightweight geometry preview proxy */
export async function POST(request: Request) {
  ...
  let geoRes: Response;
  try {
    geoRes = await fetch(`${geoUrl}/preview`, {
      method: "POST",
      headers: {
    ...
    // Fail silently for preview — don't show errors to user
    return apiError("preview_unavailable", "Preview service unavailable.", 503);
  } finally {
    clearTimeout(timeout);
  }

  if (!geoRes.ok) {
    return apiError("preview_failed", "Preview generation failed.", geoRes.status);
  }
  ...
  try {
    ...
    return apiError("preview_failed", "Invalid preview response.", 502);
  }
